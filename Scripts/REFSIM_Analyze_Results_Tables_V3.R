...
# Version 3 of this script also includes the ZINB-WAVE+DESEQ2 test variant
...
new_col_names =  c('ALDEx2-t','ALDEx2-W','ANCOM','DACOMP-t','DACOMP-ratio','DACOMP','HG','W-FLOW','W-CSS','W-TSS','Wrench','ZINB-WAVE','m1','effect')
...
replace_to = c('ALDEx2-t','ALDEx2-W','ANCOM','DACOMP-t','DACOMP-ratio','DACOMP','HG','W-FLOW','W-CSS','W-TSS','Wrench','ZINB-WAVE')
...
for(i in 1:2){
  ...
  p_fdr = ggplot(dt_current,aes(x = lambda,y=fdr,color = Method))+
    ...
    xlab(TeX("$\\lambda_{effect}$"))+scale_color_manual(values=as.character(pallete[pal_vec]))+ylab('FDR')
