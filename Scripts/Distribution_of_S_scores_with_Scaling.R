...
run_S_sim_for_scenario=function(scenario_id){
  ...
  S_scaled_for_null = NULL
  S_Scaled_for_diff_abundant = NULL
  ...
  for(sim_rep in 1:data_gen_reps){
    ...
    X_TSS = data_generation$X
    for(i in 1:nrow(X_TSS)){
      X_TSS[i,] = (X_TSS[i,]+1) / sum(X_TSS[i,])
    }
    Scaling_factors = apply(log10(X_TSS),2,sd)
    if(length(data_generation$select_diff_abundant)>1){
      ...
      S_scaled_for_null = c(ref_obj$scores[-diff_abun]/Scaling_factors[-diff_abun],S_scaled_for_null)
      S_Scaled_for_diff_abundant = c(ref_obj$scores[diff_abun]/Scaling_factors[diff_abun],S_Scaled_for_diff_abundant)
    }else{
      S_for_null = c(ref_obj$scores,S_for_null)
      S_scaled_for_null = c(ref_obj$scores/Scaling_factors,S_scaled_for_null)
  ...
  return(list(S_for_null = S_for_null,
              ...
              S_scaled_for_null = S_scaled_for_null,
              S_Scaled_for_diff_abundant = S_Scaled_for_diff_abundant))
}
...
save(parallel_res,file = '../../Results/S_sim_results_with_scaling.rdata')
  ...
  for(s in 1:length(Settings_indices)){
    temp = rbind(
      data.frame(Scenario = labels[s],Taxon = 'Diff. Abundant',
                 Scores = parallel_res[[Settings_indices[s]]]$S_Scaled_for_diff_abundant),
      data.frame(Scenario = labels[s],Taxon = 'Non-Diff. Abundant',
                 Scores = parallel_res[[Settings_indices[s]]]$S_scaled_for_null)
    )
    if(is.null(dt_gg)){
      dt_gg = temp
    }else{
...
dt_gg = dt_gg[dt_gg$Scores< 10^5,]
p = ggplot(dt_gg)+geom_histogram(aes(x = Scores,fill = Taxon,alpha = Taxon),bins = 100,position = "identity")+
  facet_wrap(~Scenario,ncol = 6)+theme_bw()+scale_y_sqrt()+scale_x_log10()+ theme(strip.text.x = element_text(size = 5), axis.text.x = element_text(size = 5)) +scale_alpha_discrete("Taxon",range = c(0.9,0.35))
p
ggsave(filename = '../../Results/Sj_distribution_CD_with_Scaling.pdf',plot = p,width = 10,height = 4)
...
p
ggsave(filename = '../../Results/Sj_distribution_signal_in_rare_with_Scaling.pdf',plot = p,width = 10,height = 4)
